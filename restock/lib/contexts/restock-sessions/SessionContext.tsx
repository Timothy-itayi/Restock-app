...
export const SessionProvider: React.FC<SessionProviderProps> = ({ children }) => {
  ...
  const loadAvailableSessions = useCallback(async () => {
    ...
    try {
      ...
      allSessions.forEach((session: RestockSession) => {
        console.log('🔍 SessionContext: Session', session.toValue().id, 'status:', session.toValue().status);
      });

      const unfinishedSessions = allSessions.filter((session: RestockSession) => {
  ...
  const refreshSessionsAfterEmailSent = useCallback(async (sentSessionId: string) => {
    ...
    setAvailableSessions(prev => prev.filter((s: RestockSession) => s.toValue().id !== sentSessionId));
  ...
  const deleteSession = useCallback(async (sessionId: string) => {
    ...
    try {
      ...
      setAvailableSessions(prev => {
        const filtered = prev.filter((s: RestockSession) => s.toValue().id !== sessionId);
