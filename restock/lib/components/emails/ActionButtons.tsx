...
import { EmailSession, EmailDraft } from '../../hooks/emails/useEmailSession';
...
export function ActionButtons({ emailSession, onSendAll }: ActionButtonsProps) {
  ...
  const isSending = emailSession.emails.some((email: EmailDraft) => email.status === 'sending');
  const areAllSent = emailSession.emails.every((email: EmailDraft) => email.status === 'sent');
