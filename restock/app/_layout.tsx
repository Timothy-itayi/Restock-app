...
export default function RootLayout() {
  ...
  useEffect(() => {
    ...
    if (!isValid) {
      ...
      console.error('❌ [RootLayout] Environment validation failed:', errorMsg);
      
      // Show alert in non-production or if debug enabled
      if (__DEV__ || process.env.EXPO_PUBLIC_ENABLE_AUTH_DEBUG === 'true') {
        Alert.alert(
          '⚠️ Configuration Error',
          `Missing required environment variables:\n\n${missing.join('\n')}\n\nCheck EAS secrets or .env file.`,
          [{ text: 'OK' }]
        );
      }
    } else {
      console.log('✅ [RootLayout] All required environment variables present');
    }
  }, []);
