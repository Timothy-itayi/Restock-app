...
export class UserContextService {
  ...
  private static contextVerificationEnabled: boolean = true;
  ...
  static async setUserContext(userId: string): Promise<void> {
    ...
    try {
      ...
      if (health.status === 'unhealthy') {
        const userError = await DatabaseHealthService.getUserFriendlyError();
        throw new Error(`Database is not ready: ${userError.message}`);
      }
      
      // Set the user context in the database session
      const { error: rpcError } = await supabase.rpc('set_current_user_id', { user_id: userId });
      ...
      if (rpcError) {
        ...
        if (rpcError.code === '42883') { // Function does not exist
          ...
          const diagnostics = await DatabaseHealthService.getDiagnostics();
          console.error('[UserContext] Database diagnostics:', diagnostics);
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('Database is not ready')) {
          throw error; // Re-throw health check error
        }
      }
   ...
   */
  private static async verifyContext(expectedUserId: string): Promise<{
    success: boolean;
    reason?: string;
    details?: any;
  }> {
  ...
  static async clearUserContext(): Promise<void> {
    try {
      if (this.currentUserId) {
        const { error } = await supabase.rpc('set_current_user_id', { user_id: null });
        if (error) {
   ...
   */
  static setContextVerification(enabled: boolean): void {
   ...
   */
  static async testSecuritySetup(): Promise<{
    ...
    try {
      ...
      const { error: rpcError } = await supabase.rpc('set_current_user_id', { user_id: 'test' });
  ...
  /**
   * Get user-friendly error messages for database issues
   */
  static async getUserFriendlyError() {
    return DatabaseHealthService.getUserFriendlyError();
  }

  /**
   * Check if the app can function with current database health
   */
  static async canAppFunction() {
    return DatabaseHealthService.canAppFunction();
  }
}
